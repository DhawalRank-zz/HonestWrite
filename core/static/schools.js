...
function populateSchool( stypeElementId, sElementId ) {
	...
	schoolElement.options[0] = new Option('Select School', '-1');
	schoolElement.selectedIndex = 0;

	var school_arr = s_a[selectedSIndex].split("|");
	console.log(school_arr);
	for (var i = 0; i < school_arr.length; i++) {
		console.log(school_arr[i]);
		schoolElement.options[schoolElement.length] = new Option(school_arr[i], school_arr[i]);
	}
    document.getElementById(sElementId).style.display = "inherit";
    document.getElementById(sElementId).className = "input-field";
    console.log(document.getElementById(sElementId));
}
function populateStype(stypeElementId){
    ...
    console.log(document.getElementById(stypeElementId));
}